...
public class ArtistMediaService {
    ...
    /**
     * Получает список медиафайлов для указанной работы с проверкой.
     *
     ...
     */
    public Page<MediaDto> getArtistWorkMedias(Long userId, Long workId, Pageable pageable) {
        Long artistId = getArtistProfileByUserId(userId).getId();

        // Проверяем принадлежность работы художнику
        validateWorkOwnership(artistId, workId);
        
        // Получаем медиафайлы с пагинацией
        return mediaRepository.findByWorkIdOrderByCreatedAt(workId, pageable)
                .map(media -> {
    ...
    /**
     * Получает список медиафайлов текущего художника для указанной работы с проверкой.
     *
     * @param artistId ID художника
     * @param workId ID работы
     * @param pageable параметры пагинации
     ...
     * @throws ResourceNotFoundException если работа не найдена или не принадлежит художнику
     */
    public Page<MediaDto> getCurrentArtistWorkMedias(Long workId, Pageable pageable) {
