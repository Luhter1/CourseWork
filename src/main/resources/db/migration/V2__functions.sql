...
AS $$
BEGIN
    INSERT INTO art2art_program_views_log (program_id, viewed_at) VALUES (p_program_id, now());
END;
$$;
...
AS $$
BEGIN
    INSERT INTO art2art_residence_views_log (residence_id, viewed_at) VALUES (p_residence_id, now());
END;
$$;

----------------------------------------------------------------------
-- создаёт запись в art2art_programs и art2art_program_stats
----------------------------------------------------------------------
...
BEGIN
    ...
    PERFORM 1 FROM art2art_residence_details rd WHERE rd.id = p_residence_id;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Residence id % does not exist', p_residence_id;
    END IF;

    -- проверка, что пользователь владелей резиденции
    SELECT user_id INTO v_residence_owner FROM art2art_residence_details WHERE id = p_residence_id;
    ...
    -- вставляем программу
    INSERT INTO art2art_programs (
        residence_id, title, description, goals, conditions,
        deadline_apply, deadline_review, deadline_notify,
        duration_days, budget_quota, people_quota, 
        is_published, created_at, updated_at
    )
    VALUES (
        p_residence_id, p_title, p_description, p_goals, p_conditions,
    ...
    INSERT INTO art2art_program_stats (program_id, views_count, applications_count, created_at, updated_at)
...
BEGIN
    -- проверка, что программа существует, опубликована и дедлайн не прошёл ХХХ
    SELECT * INTO v_program FROM art2art_programs WHERE id = p_program_id FOR SHARE;
    IF NOT FOUND THEN
    ...
    IF v_program.is_published IS NOT TRUE THEN
        RAISE EXCEPTION 'Program is not published';
    END IF;

    -- проверка роли художника
    PERFORM 1 FROM art2art_users WHERE id = p_artist_user_id AND role = 'ROLE_ARTIST';
    IF NOT FOUND THEN
        RAISE EXCEPTION 'User % is not an artist', p_artist_user_id;
    END IF;

    -- проверка уникальности для читаемой ошибки
    SELECT id INTO v_existing FROM art2art_application_requests WHERE artist_id = p_artist_user_id AND program_id = p_program_id;
    IF FOUND THEN
...
DECLARE
    v_app RECORD;
BEGIN
    SELECT * INTO v_app FROM art2art_application_requests WHERE id = p_application_id FOR UPDATE;
    IF NOT FOUND THEN
...
BEGIN
    ...
    -- проверка роли
    PERFORM 1 FROM art2art_users WHERE id = p_expert_user_id AND role = 'ROLE_EXPERT';
    IF NOT FOUND THEN
        RAISE EXCEPTION 'User % is not an expert', p_expert_user_id;
    END IF;

    -- проверка прав назначающего
    SELECT rd.user_id INTO v_owner FROM art2art_residence_details rd WHERE rd.id = v_program.residence_id;
    IF p_assigner_user_id <> v_owner THEN
        PERFORM 1 FROM art2art_users WHERE id = p_assigner_user_id AND role = 'ROLE_SUPERADMIN';
        IF NOT FOUND THEN
    ...
    -- проверка уникальности для читаемой ошибки
    SELECT id INTO v_existing FROM art2art_program_experts WHERE program_id = p_program_id AND user_id = p_expert_user_id;
    IF FOUND THEN
    ...
    INSERT INTO art2art_program_experts (program_id, user_id, assigned_at, created_at)
...
BEGIN
    ...
    FROM art2art_application_evaluations ae
    JOIN art2art_application_requests ar ON ar.id = ae.application_id
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM art2art_program_experts pe WHERE pe.program_id = v_app.program_id AND pe.user_id = p_expert_user_id
    ) INTO v_assigned;
    ...
    SELECT id INTO v_eval_id FROM art2art_application_evaluations WHERE application_id = p_application_id AND expert_id = p_expert_user_id;
    IF FOUND THEN
        RAISE EXCEPTION 'Expert % already evaluated application % (evaluation id %)', p_expert_user_id, p_application_id, v_eval_id;
    END IF;

    -- вставка оценки
    INSERT INTO art2art_application_evaluations (application_id, expert_id, score, comment, created_at)
    ...
    FROM art2art_program_experts pe
    WHERE pe.program_id = v_app.program_id
      AND NOT EXISTS (
          SELECT 1 FROM art2art_application_evaluations ae WHERE ae.application_id = p_application_id AND ae.expert_id = pe.user_id
      );
...
DECLARE
    v_avg NUMERIC;
BEGIN
    SELECT avg(score::numeric) INTO v_avg FROM art2art_application_evaluations WHERE application_id = p_application_id;
